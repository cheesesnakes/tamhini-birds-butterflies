...
tstats <- function(x){
  x%>%
    rename('1998-2001' = estimate1,
         '2016-2017' = estimate2)%>%
    select(`1998-2001`, `2016-2017`, parameter, statistic, p.value)
}
...
More species of bothe taxa were encountered in current study compared to the previous study.
...
veg_birds <- abn%>%
  ...
  select(sample, Specific.Epithet, n)%>%
  ungroup()%>%
...
CSI.T <- abn%>%
  select(Study, Transect, Taxa, Specific.Epithet)%>%
  group_by(Study, Taxa, Transect)%>%
  distinct()%>%
  ...
  select(Study, Transect, Taxa, Specific.Epithet, TSI)%>%
...
CSI.H <- abn%>%
  ...
  group_by(Study, Taxa, Transect)%>%
  distinct()%>%
  left_join(species, by = c("Study", "Taxa", "Specific.Epithet"))%>%
  select(Study, Transect, Taxa, Specific.Epithet, HSI)%>%

...
host <- read.csv("./data/butterfly host plants.csv") #host plant data

host <- host%>%
  rename(Common.Name = ï..Common.Name)
...
CSI.T <- abn%>%
  ...
  group_by(Study, Taxa, Transect)%>%
  distinct()%>%
  left_join(species_TSI, by = c("Species" = "Common.Name"))%>%
...
habitat <- abn%>%
  left_join(species)%>%
  group_by(Study, Taxa, Transect, Habitat.Preference)%>%
  summarise(n = sum(Abundance, na.rm = T))%>%
  #group_by(Study, Taxa, Transect)%>%
  mutate(N = sum(n))%>%
  #filter(Guild == "Generalist" | Guild == "Omnivore")%>%
...
habitat%>%
  ggplot(aes(reorder(Habitat.Preference, prop.gen), prop.gen, col = Study, fill = Study))+
  geom_boxplot(width = 0.25, alpha = 0.5, position = position_dodge(preserve = "single"))+
  labs(y = "Relative Proportion", x = "Habitat Preference")+
  scale_x_discrete(guide = guide_axis(n.dodge = 2))+
  scale_color_brewer(palette = "Set1")+
  facet_wrap(~Taxa, scales = "free_x", ncol = 1)
```

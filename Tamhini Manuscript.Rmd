...
abn <- read.csv("./data/Tamhini_birds_butterflies.csv")
transects <- read.csv("./data/Transect Co-ordinates.csv")
...
species <- abn%>%
  ...
  mutate(N = sum(n),
         rel.prop = n/N)%>% # relative proportion in each habitat
  #filter(rel.prop > 0.01)%>% # species that make up more than 1% of a community
  complete(nesting(Study, Species), Habitat, fill = list(rel.prop = 0))%>%
  group_by(Study, Species)%>%
  mutate(HSI = sd(rel.prop)/mean(rel.prop))%>% #Julliard et al. 2006
  filter(rel.prop > 0)%>%
  mutate(#Hab.occ = length(unique(Habitat)), #number of habitats occupied
         Habitat.Preference = ifelse(length(unique(Habitat)) > 1, 
                                     "Generalist", 
                                     as.character(Habitat)) # classifying habitat preference
         )%>%
  ungroup()%>%
...
diet_sp <- abn%>%
  ungroup()%>%
  select(Taxa, Species, Guild)%>%
  distinct(.keep_all = F)

write.csv(diet_sp, "./Data/species_diet.csv")
```
